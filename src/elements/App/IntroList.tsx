...
const IntroList = memo(({ title, data, renderItem, ...rest }: IIntroListProps) => {
  ...
  const basicRenderItem = ({ item, index }: { item: IntroButtonProps; index: number }) => {
    const _onPress = () => {
      item.onPress();
    ...
    return (
      <TouchableOpacity style={styles.button} onPress={_onPress} activeOpacity={0.7}>
